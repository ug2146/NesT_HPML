...
def get_model(key, pretrained = False, preset_parameters=None):

    if "analog" in key.lower() and "resnet" in key.lower(): return resnet34(preset_parameters)

    if "nest_t" in key.lower() or "nest-t" in key.lower() and "analog" in key.lower(): return nest_tiny_analog(pretrained, preset_parameters)
